...
def fix_field_def(match: re.Match) -> str: var_name = match.group(1)
    ...
    if ", " in type_hint and not ("[" in type_hint or "(" in type_hint):
        ...
        if field_args: returnf"    {var_name}: {type_hint} = field({field_args.strip()})"
        ...
        paren_level -= 1
        elif char == ", " and paren_level == 0: param_list.append("".join(current_param).strip())
        current_param = []
        continue
...
def fix_method(match: re.Match) -> str: indent = match.group(1)
    ...
    if "self" in method_def and "(" in method_def: parts = method_def.split("(", 1)
        ...
        if param_list[0].strip() == "self":
            ...
            else: param_list = ["self"] + [p for p in param_list if p and "self" not in p]
