import os
    ...
    with open(os.path.join(os.path.dirname(__file__), "src/models/text_to_anything.py"), "r") as f:
    content = f.readlines()
    ...
    fixed_content.append(
    f"{method_indent}# Truncate or pad to max_length\n"
    )
    fixed_content.append(
    ...
    fixed_content.append(
    f"{method_indent}elif len(tokens) < self.max_length:\n"
    )
    fixed_content.append(
    f"{method_indent}    tokens.extend([self.pad_token] * (self.max_length - len(tokens)))\n"
    )
    fixed_content.append(
