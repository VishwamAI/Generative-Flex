...
def fix_specific_patterns(content: str) -> str:
    ...
    for line in content.split('\n'):
        ...
        if stripped.startswith('def '):
            ...
            if method_match:
                ...
                if method_name.startswith('test_'):
                    ...
                    lines.append(f'{indent}    """Test method docstring."""')
        ...
        if 'torch.device' in stripped:
            lines.append(f'{indent}def test_device():')
            lines.append(f'{indent}    """Test device setup."""')
            lines.append(f'{indent}    device = "cuda" if torch.cuda.is_available() else "cpu"')
            continue

        # Fix config test setup
        if 'config.__post_init__()' in stripped:
            lines.append(f'{indent}def test_config():')
            lines.append(f'{indent}    """Test configuration setup."""')
            lines.append(f'{indent}    config = MathConfig()')
            continue
