...
def fix_function_definition(content: str) -> str:
        ...
        if params.strip():
            ...
            for param in param_list:
                if ":" in param and "=" in param:
                    ...
                    fixed_param = f"{name.strip()}: {type_and_default[0].strip()} = {type_and_default[1].strip()}"
                elif ":" in param:
                    name, type_hint = param.split(":", 1)
                    fixed_param = f"{name.strip()}: {type_hint.strip()}"
                else:
                    fixed_param = param
    ...
    content = re.sub(pattern, fix_params, content, flags=re.DOTALL)
...
def fix_class_definition(content: str) -> str:
    ...
    content = re.sub(pattern, fix_class_def, content)
    ...
    for line in lines:
        ...
        if in_dataclass and ":" in line and "=" in line:
            ...
            if "field(" in type_and_default:
                ...
                type_part = type_and_default.split("=", 1)[0].strip()
                field_part = type_and_default.split("=", 1)[1].strip()
                fixed_line = f"    {field_name}: {type_part} = {field_part}"
            else:
    ...
    for line in lines:
        if line.strip().startswith(("import ", "from ")):
            ...
            if parts[0] == "from":
                ...
                fixed_line = f"from {module} import {imports}"
            else:
                # Handle 'import ...'
