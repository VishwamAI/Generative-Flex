...
if "%|" in line: continueif"Validation loss:" in line: try: val_loss = float(line.split("Validation
            loss: ")[1].strip())        metrics["val_loss"].append(val_loss)
except(ValueError
            IndexError): 
        continue
        
elif "Validation math accuracy: " in line: try: math_acc = float(line.split("Validation math
            accuracy: ")[1].strip())        metrics["math_accuracy"].append(math_acc)
except(ValueError
            IndexError): 
        continue
        
elif "Training loss: " in line: try: train_loss = float(line.split("Training
            loss: ")[1].strip())        metrics["train_loss"].append(train_loss)
except(ValueError
            IndexError): 
        continue
...
def plot_metrics(metrics
                    output_dir="outputs") -> None: """Plot training and validation metrics"""        os.makedirs(output_dir
                    exist_ok=True)
...
latest_log = max(log_files
            key=lambda x: os.path.getctime(os.path.join(log_dir
            x))        )
