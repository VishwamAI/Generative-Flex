...
def fix_math_modules(content):
    ...
    content = re.sub(r'class\s+MathConfig\s*(?:\([^)]*\))?:', 'class MathConfig(object):\n    pass', content)
    ...
    content = re.sub(r'class\s+MathConfig\s*\(\s*\):', 'class MathConfig(object):', content)

    # Add minimal docstrings
    content = re.sub(r'(class\s+\w+(?:\([^)]*\))?:)', r'\1\n    """."""', content)
    content = re.sub(r'(def\s+\w+\([^)]*\):)', r'\1\n    """."""', content)
