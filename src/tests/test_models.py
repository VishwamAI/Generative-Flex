...
def test_image_model_training(image_model):
    ...
    output = image_model.apply(variables, images, training=True, rngs={'dropout': dropout_rng})
...
def test_audio_model_forward(audio_model):
    ...
    expected_samples = ((AUDIO_SAMPLES - audio_model.frame_size) // audio_model.hop_length + 1) * audio_model.hop_length
...
def test_audio_model_training(audio_model):
    ...
    output = audio_model.apply(variables, audio, training=True, rngs={'dropout': dropout_rng})
...
def test_video_model_init(video_model):
    rng = jax.random.PRNGKey(0)
    video = jnp.ones((BATCH_SIZE, VIDEO_FRAMES, *IMAGE_SIZE, CHANNELS), dtype=jnp.float32)
...
def test_video_model_training(video_model):
    ...
    output = video_model.apply(variables, video, training=True, rngs={'dropout': dropout_rng})
