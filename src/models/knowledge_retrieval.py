...
if isinstance(inputs, dict):
    ...
    for mod, data in inputs.items():
        ...
        else: raise ValueError(f"No valid modalities found in input. Expected one of {{self.config.modalities}}")
        ...
        if context is not None: if len(context.shape)  = = 2: context = context[:, None, :]  # Add sequence dimension
...
if self.update_counter >= self.config.update_frequency: if self.knowledge_retriever is not None:
...
def __init__(self):,    inputs: Dict[str, jnp.ndarray],
