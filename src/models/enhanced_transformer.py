...
class MultiModalEmbedding(nn.Module):
    ...
    logger.info(
    f"Text embeddings shape: {text_embeddings.shape}"
    )
    ...
    # Reshape to match text embedding dimensions(
    batch_size,
    seq_length,
    hidden_size
    )
    ...
    logger.info(
    f"Position embeddings shape: {position_embeddings.shape}"
    )
    ...
    logger.info(
    f"Token type embeddings shape: {token_type_embeddings.shape}"
    )
    ...
    token_type_embeddings = token_type_embeddings.expand(
    batch_size, -1, -1
    )
    ...
    logger.error(
    f"Error in MultiModalEmbedding forward pass: {str(e)}"
    )
    raise
...
class FlashAttention(nn.Module):
    ...
    "bhqk,bkhd->bqhd", attention_weights, v
    )
...
class ExpertLayer(nn.Module):
    ...
    expert_outputs * routing_weights.unsqueeze(
    -1),
    dim=2
    )
    )
...
class EnhancedTransformerBlock(nn.Module):
    ...
    nn.GELU(
    ) if config.hidden_act == "gelu" else nn.ReLU(),
    ...
    logger.info(
    f"Attention config: num_heads={num_heads}, head_dim={head_dim}"
    )
    ...
    logger.info(
    f"Final attention mask shape: {attention_mask.shape}"
    )
    ...
    logger.info(
    f"Flash attention output shape: {attention_output.shape}"
    )
    else:
    ...
    x = x +
    (
    self.dropout(expert_output) if training else expert_output
    )
    ...
    x = x +
    (self.dropout(ffn_output) if training else ffn_output)
    ...
    logger.error(
    f"Error in EnhancedTransformerBlock forward pass: {str(e)}"
    )
    raise
...
class EnhancedTransformer(PreTrainedModel):
    ...
    self.logger.info(
    "Getting embeddings from multimodal embedding layer"
    )
    ...
    if use_cache:
    self.logger.warning(
    "Gradient checkpointing disables cache"
    )
    use_cache = False
    ...
    self.logger.info(
    f"Block {i +
    1} output shape: {hidden_states.shape}"
    )
    self.logger.info(
    f"Memory after block {i +
    1}: {torch.cuda.memory_allocated() / 1024**2:.2f}MB"
    )
    else:
    self.logger.info(
    ...
    self.logger.error(
    f"Error in transformer block {i +
    1}: {str(e)}"
    )
    raise
    ...
    self.logger.info(
    f"Final hidden states shape: {hidden_states.shape}"
    )
    ...
    return(
    (loss,
    ) + output) if loss is not None else output
    )

    return {

    # Will be None during evaluation if no labels provided
    "logits": logits,
    ...
    self.logger.error(
    f"Error in EnhancedTransformer forward pass: {str(e)}"
    )
    raise
    ...
    # Update inputs for next iteration             if "text" in current_input:
