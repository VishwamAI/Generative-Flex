...
class SymbolicMathProcessor(nn.Module):
    ...
    def embed_expression(self, tokens: List[str], hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for token in tokens:
            if token in ['+', '-', '*', '/']:
                op_idx = ['+', '-', '*', '/'].index(token)
                embedding = self.operator_embeddings[op_idx].unsqueeze(0).expand(batch_size, -1)
            else:
    ...
    def forward(self, hidden_states: torch.Tensor, expressions: List[str]) -> torch.Tensor:
...
class MathematicalNotationProcessor(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, notation_indices: torch.Tensor) -> torch.Tensor:
