...
class VideoEmbedding(nn.Module):    """Video to embedding conversion."""
    ...
    hidden_dim: int, patch_size: Tuple[int, int, int]# (time, height, width)
    dtype: Any = jnp.float32
    @nn.compact
    def __call__(self, video) -> None: b, t, h, w, c = video.shape:        patches = jnp.reshape(video, (        b, t // self.patch_size[0], h // self.patch_size[1], w // self.patch_size[2], *self.patch_size, c))
    ...
    class VideoGenerationModel(nn.Module):        """Transformer-based video generation model."""
        ...
        @nn.compact
        def __call__(self, inputs, training: boo, l = True) -> None: b, t, h, w, c = inputs.shape:            assert(t == self.video_size[0]            and h == self.video_size[1]
        ...
        def generate(self):                rng: Any,                prompt: Optional[jnp.ndarray] = None,                num_frames: int = 16):                    """Generate video frames."""
                    if prompt is None: rng, init_rng = jax.random.split(rng)                    prompt = jax.random.normal(init_rng, (1, 1, self.video_size[1], self.video_size[2], self.channels))

                    generated = prompt
                    while generated.shape[1] < num_frames: next_frame = self.apply({"params": self.params}, generated, training=False)                    generated = jnp.concatenate([generated, next_frame[:, -1: ]], axis=1)
