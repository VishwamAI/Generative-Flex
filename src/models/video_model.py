...
class VideoEmbedding(nn.Module):    """Video to embedding conversion."""
    ...
    @nn.compact
    def __call__(self, video) -> None: b, t, h, w, c = video.shape:        patches = jnp.reshape(video, (
    ...
    class VideoGenerationModel(nn.Module):        """Transformer-based video generation model."""
        ...
        @nn.compact
        def __call__(self, inputs, training: boo, l = True) -> None: b, t, h, w, c = inputs.shape:            assert(t == self.video_size[0]
        ...
        def generate(self):                rng: Any,
                ...
                num_frames: int = 16):
                    ...
                    generated = prompt
                    while generated.shape[1] < num_frames: next_frame = self.apply({"params": self.params}, generated, training=False)
