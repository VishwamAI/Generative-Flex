...
class AudioEmbedding(nn.Module):    """Audio signal to embedding."""
    ...
    @nn.compact
def __call__(self
        x) -> None: """Convert audio signal to embeddings."""        batch_size
...
frames = frames * window[None
    ...
    class AudioGenerationModel(nn.Module):        """Transformer-based audio generation model."""
        ...
        @nn.compact
def __call__(self
            x) -> None: """Forward pass of the audio generation model."""            batch_size
...
output = output.at[: 
                    indices].add(x * window[None
                    None
                    : ])
                # Normalize by window overlap
                divisor = jnp.zeros_like(output)
divisor = divisor.at[: 
                    indices].add(window[None
                    None
                    : ] ** 2)                output = jnp.where(divisor > 1e-8
                    output / divisor
                    output)
