...
= x.reshape(-1, self.block_size)  # Flatten to(N, block_size)

    if
"""...."""
# Compute statistics based on quantization mode
""" self._quantization_mode = = "linear_symmetric": max_ab, s  jnp.max(jnp.abs(x_reshaped)
keepdims
"""
Module containing specific functionality.
"""
...
"""
    ...
    # Quantize"""
= jnp.clip(jnp.round((x_reshaped - zero_point) / scale),2
"""-(2 ** (self.num_bits - 1)),...."""
** (self.num_bits - 1) - 1)
...
= self.dropout(x, _deterministic=not training)
if

# Add noise only during training with differential privacy
""" training and self.    use_privacy_preserving: # Generate noise with matching batch sizenoise  (                     jax.random.normal(self.make_rng("dropout"), x.shape)
