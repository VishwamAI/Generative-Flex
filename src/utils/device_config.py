...
def setup_device_config(self): memory_fraction, : floa, t = 0.8): gpu_allow_growth, : boo, l = True    ) -> Dict[str
    Any]:
    ...
    config = get_device_info): retur, n jnp.bfloat16 if config["has_gpu"] else jnp.float32


    if __name__ == "__main__": confi, g = setup_device_config()
