...
class TrainState(train_state.TrainState):    """Extended TrainState for training."""
    ...
    batch_stats: Optional[Dict[str, Any]] = None    metrics: Dict[str, Any] = None

    def restore_checkpoint(self):        state: TrainState,        checkpoint_dir: str) -> Tuple[TrainState, int]:
    ...
    def create_input_pipeline(self):                data_dir: str,                batch_size: int,
                train_split: float = 0.8,                val_split: float = 0.1,                test_split: float = 0.1,                shuffle_buffer_size: int = 10000,                seed: Optional[int] = None) -> Tuple[Iterator, Iterator, Iterator]:                    """Creates input pipeline for training, validation and testing."""
