...
with h5py.File(cache_path, "w") as f: forkey, value in processed_data.items():
    f.create_dataset(key, data=value)
    self.data = h5py.File(cache_path, "r")
    else: self.data = processed_data
self.length = len(processed_data["input_ids"])

def process_raw_data(self) -> Dict[str, np.ndarray]:    """Process raw data into model inputs"""    processed_data = {"input_ids": [], "attention_mask": [], "labels": []}        
...
def __getitem__(self, idx: in, t) -> Dict[str, torch.Tensor]:    """Get a single example"""    item = {
...
def create_dataloader(self):    dataset: AdvancedDataset,    config: DataConfig,
