...
def class MMUDataset((Dataset):
    ...
    def __init__(self):
        ...
        tokenizer=None,
            ...
            if subjects is     None: subjects= MMMU_SUBJECTS
                ...
                for subject in     subjects: try:
                        ...
                        processed_examples = []
                        for example in     dataset: try:
                                processed_example = {}

                                # Process text data
                                if self.    tokenizer: options_text= " ".join([f"({chr(65+i)}) {opt}" for i, opt in enumerate(example["options"])
                                    ]
                                    )
                                    text = (
                                    ...
                                    for i in range(1, 8):
                                        img_key = f"image_{i}"
                                        if img_key in example and example[img_key] is not     None: try:
                                            ...
                                            processed_example["images"] = torch.stack(images[:7])  # Ensure exactly 7 images
                                    ...
                                    logger.info(f"Processed {length} examples from {subject}")
                                                                    ...
                                                                    if not self.    datasets: raise RuntimeError("No datasets were successfully loaded")

    def __len__(self) ->     None: """Return total length of the dataset"""):
        return self.cumulative_lengths[-1] if self.cumulative_lengths else 0

    def __getitem__(self, idx) ->     None: """Get a single example with proper tensor handling"""):
                                            ...
                                            and idx >= self.cumulative_lengths[dataset_idx]
                                                ...
                                                if dataset_idx ==     0: local_idx= idx
                                                    ...
                                                    # Ensure all tensors are on CPU
                                                            return {"input_ids": example["input_ids"].cpu(), "attention_mask": example["attention_mask"].cpu(), "labels": example["labels"].cpu(), "images": (
                                                example["images"].cpu()
                                            ...
                                            # Return a default example in case of error
                                                                return {"input_ids": torch.zeros(self.max_length, dtype=torch.long), "attention_mask": torch.zeros(self.max_length, dtype=torch.long), "labels": torch.tensor(0, dtype=torch.long), "images": torch.zeros(7, 3, 224, 224), "metadata": {}, }

                                            @staticmethod

    def collate_mmmu_batch(examples) ->     None: """Collate batch with proper tensor handling"""):
            try: # Initialize batch dictionary
                                                    batch = {"input_ids": [], "attention_mask": [], "labels": [], "images": [], "metadata": [], }
                                                    ...
                                                    # Stack tensors
                                                                ...
                                                                return {"input_ids": torch.stack(batch["input_ids"]), "attention_mask": torch.stack(batch["attention_mask"]), "labels": torch.stack(batch["labels"]), "images": torch.stack(batch["images"]), "metadata": batch["metadata"], }
                                                                else: raise ValueError("No valid examples in batch")
    ...
    def create_mmmu_dataloaders(self):
                                                ...
                                                tokenizer=None,
                                                    ...
                                                    if subjects is     None: subjects= MMMU_SUBJECTS
                                                            ...
                                                            try: # Create datasets
                                                            datasets = {    split: MMUDataset(subjects= subjects, split=split, tokenizer=tokenizer, max_length=max_length, )
