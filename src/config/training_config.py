...
# Model configuration
model_name: str = (field(default="facebook/opt-125m"),)subjects: List[str] = field(default_factory=list)batch_size: int = (field(default=4),)learning_rate: float = (field(default=2e-5),)num_epochs: int = (field(default=5),)gradient_accumulation_steps: int = (field(default=8),)max_grad_norm: float = (field(default=1.0),)warmup_steps: int = (field(default=100),)device: str = (field(default="cuda"),)fp16: bool = field(default=True)
