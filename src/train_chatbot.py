...
def load_data(file_path: str = 'data/chatbot/training_data_cot.json') -> List[Dict[str, str]]:
...
def prepare_batch(conversations: List[Dict[str, str]], vocab: Dict[str, int],
                 batch_size: int, max_length: int) -> tuple:
    inputs = []
    targets = []
...
def main():
    ...
    inputs, targets = prepare_batch(conversations, vocab,
    ...
    for epoch in range(num_epochs):
        ...
        state, loss = train_step(state, jnp.array(inputs),
                               jnp.array(targets), train_rng)
