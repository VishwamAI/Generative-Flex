...
def main():
    ...
    with open(config_path) as f:
        ...
        accelerator = Accelerator(gradient_accumulation_steps=config["training"][
        ...
        if config["training"]["seed"] is not None:
            ...
            # Initialize trainer
            trainer = AcceleratedTrainer(model=model,
                accelerator=accelerator,
                config=config["training"],
                output_dir=config["training"]["output_dir"],)

            # Create dataloaders
            train_dataloader, eval_dataloader = create_dataloaders(batch_size=config["training"]["batch_size"],
            ...
            if config["training"]["push_to_hub"] and config["training"]["hub_model_id"]:
                trainer.push_to_hub(repo_id=config["training"]["hub_model_id"],
