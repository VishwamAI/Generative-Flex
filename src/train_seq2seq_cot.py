...
class SimpleSeq2SeqModel(nn.Module):
    ...
    def loss_fn(self, params) -> None: logit, s = model.apply({"params": params, }, x):                    return optax.softmax_cross_entropy_with_integer_labels(                logits=logits[:, : y.shape[0]], labels=y                ).mean()
