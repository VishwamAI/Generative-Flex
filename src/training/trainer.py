...
import torch
    ...
    logger
    """Base trainer implementation.""" = logging.getLogger(__name__)


eval_dataloader
    """Base trainer class.""": Optional[DataLoader] = None
...
save_steps: int = 1000
    ...
    self
    """Initialize the trainer.""".model = model
...
if self._step % self.logging_steps = = 0: self.log_metrics({"loss": total_los, s / self.logging_steps})                        total_loss = 0 loss
    """Perform a single training step.     outputs = self.model(**batch)""" = outputs.lossif
    """
loss.backward()
