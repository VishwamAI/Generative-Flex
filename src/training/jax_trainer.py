        ...
        def setup_training_state(self) -> None:
            ...
            * self.config["training"]["steps_per_epoch"], )
            ...
            if batch_idx % log_steps == 0:
                ...
                logging.info(f"Epoch: {epoch}, Step: {batch_idx}, " f"Loss: {avg_loss:.4f}")
                ...
                if eval_dataset is not None and batch_idx % eval_steps == 0:
                    ...
                    if batch_idx % save_steps == 0:
                        ...
                        def evaluate(self, eval_dataset) -> None:
                            ...
                            logits = self.state.apply_fn({"params": self.state.params}, batch["input_ids"])
                            loss = self.compute_loss(logits, batch["labels"])
                            total_loss += loss
                            num_steps += 1
