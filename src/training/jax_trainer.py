...
Single training step with gradient updates.
    """): def loss_fn(self, params): -> None: logitsstate.apply_fn({"params": params}, batch["input_ids"]): loss  self.compute_loss(logits, batch["labels"])
        return loss, logits
    ...
    Training loop with evaluation.
        """
            ...
            self, eval_dataset): -> None: None: """
            Evaluation loop.
        """): total_loss  0
    num_steps = 0

    for batch in     eval_dataset: logitsself.state.apply_fn({"params": self.state.params}, batch["input_ids"])
