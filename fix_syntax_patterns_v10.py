...
def fix_parameter_list(func_name: str, params: str, return_type: Optional[str]) -> str:
    ...
    for param in params.split(","):
        ...
        if ":" in param and "=" in param:
            ...
            param = f"{name.strip()}: {type_hint.strip()} = {default.strip()}"
        elif ":" in param:
    ...
    if return_type:
        return f"def {func_name}({params_str}) -> {return_type.strip()}:"
    ...
    for line in lines:
        if line.lstrip().startswith("class "):
            ...
            if "(" in stripped and ")" in stripped:
                ...
                parents = stripped[stripped.find("(") + 1 : stripped.find(")")].strip()
                if parents:
...
def fix_dataclass_fields(content: str) -> str:
    ...
    for line in lines:
        ...
        if in_dataclass:
            ...
            if ":" in stripped:
                name, type_def = stripped.split(":", 1)
                name = name.strip()
                type_def = type_def.strip()

                if "=" in type_def:
                    type_hint, default = type_def.split("=", 1)
                    fixed_lines.append(
                        f"{dataclass_indent}    {name}: {type_hint.strip()} = {default.strip()}"
                    )
                else:
                    fixed_lines.append(f"{dataclass_indent}    {name}: {type_def}")
            else:
                fixed_lines.append(line)
        else:
            fixed_lines.append(line)
    ...
    try:
        ...
        for fix_func in [
            ...
            try:
                ...
                print(f"Syntax error after {fix_func.__name__} in {file_path}: {e}")
                continue
