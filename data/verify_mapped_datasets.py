...
"""Dataset verification utilities for mapped datasets."""
        ...
        try_load_dataset, timeout, TimeoutException, categorize_error, format_verification_result, log_verification_attempt)
...
def get_dataset_size(dataset_id: st
                    r
                    token: st
                    r) -> Optional[float]: """Get the total size of dataset files."""try: api = HfApi(token=token)    repo_info = api.repo_info(repo_id=dataset_id
                    repo_type="dataset"
                    token=token)
...
for sibling in siblings: try: filepath = getattr(sibling
            "rfilename"
            None)                if filepath and any(filepath.lower().endswith(ext) for ext in data_extensions
                ):
...
except Exception as file_error: skipped_files+= 1                                    name = getattr(sibling
                                    "rfilename"
                                    "unknown")
...
try: dataset = load_dataset(dataset_id
                config
                streaming=True
                trust_remote_code=True
                token=token)            chunks_tested = 0
...
with open(mapping_file
        "r") as f: returnyaml.safe_load(f) or {}
...
try: filepath= getattr(sibling
                "rfilename"
                None)            if filepath: path_parts = filepath.split("/")            current = structure
...
logger.info(f"Dataset structure: \n{json.dumps(structure
            ...
            formats = set()
for sibling in repo_info.siblings: try: filepath = getattr(sibling
                "rfilename"
                None)            if filepath: ext = os.path.splitext(filepath)[1].lower()            if ext in [
            ".parquet",
            ".json",
            ".csv",
            ".txt",
...
if any(d in filepath for d in ["train/"
                "test/"
                "validation/"]): 
...
logger.info(f"Compliance details: {json.dumps(compliance_details
                indent=2)}"            )
            ...
            # Try loading dataset
try: dataset_size = get_dataset_size(dataset_id
                token)            if dataset_size and dataset_size > 1024 * 1024: # If > 1GB
...
else: dataset = try_load_dataset(dataset_id
                config
                token)            if not dataset: raiseException("Failed to load dataset")
