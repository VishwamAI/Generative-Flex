...
logger.warning(
f"Failed to process file {name}: {str(file_error)}"
)
...
if psutil.Process().memory_percent() > 70: 
# Memory threshold
cleanup_memory()

# Get next chunk
chunk = list(
itertools.islice(dataset["train"],
chunk_size
)
...
logger.warning(
f"Failed to detect file format: {str(e)}"
)
...
compliance_details["has_standard_dirs"] = any(
dir in structure for dir in expected_dirs
)
...
and (
compliance_details["has_standard_dirs"]
...
logger.info(
f"Documentation compliance: {result['organization']['documentation_compliance']}"
)
logger.info(
f"Compliance details: {json.dumps(
compliance_details,                             indent=2)}"
)
)

except Exception as e:
logger.warning(
f"Failed to analyze dataset organization: {str(e)}"
)


# Check dataset size
dataset_size_kb = get_dataset_size(
dataset_id,
token
)
if (
dataset_size_kb and dataset_size_kb > 1000000

# If larger than 1GB
logger.info(
f"Large dataset detected ({dataset_size_kb/1000000:.1f} GB). Using chunked loading."
)
...
try:
logger.info(
f"Attempting to load specific config in chunks: {config}"
)
success, error, info = load_dataset_in_chunks(
dataset_id, config, token
)
...
"error_category": (
categorize_error(error) if error else None
),
"info": info,
}
...
logger.info(
f"Successfully verified large dataset {dataset_id} with config {config}"
)
return local_dir, result

except Exception as e:
logger.warning(
f"Chunked config-specific load failed for {dataset_id} with {config}: {str(e)}"
)
cleanup_memory()
...
try:
logger.info(
f"Attempting to load specific config: {config}"
)
...
result["status"] = "verified"
logger.info(
f"Successfully verified {dataset_id} with config {config}"
)
return local_dir, result

except Exception as e:
logger.warning(
f"Config-specific load failed for {dataset_id} with {config}: {str(e)}"
)
cleanup_memory()
...
logger.warning(
"Skipping non-streaming strategy due to high memory usage"
)
if not streaming:
continue
...
try:
logger.info(
f"Attempting {strategy_name} load for {dataset_id} (retries left: {retries})"
)
success, error, info = try_load_dataset(
dataset_id,
streaming=streaming,
...
result["status"] = "verified"
logger.info(
f"Successfully verified {dataset_id} with {strategy_name}"
)
return local_dir, result

# Break if load completed without error

except Exception as e:
logger.warning(
f"Basic load failed for {dataset_id} with {strategy_name}: {str(e)}"
)
cleanup_memory()
retries -= 1
if retries > 0:
...
and dataset_info.config_names
):
configs = dataset_info.config_names
logger.info(
f"Found configurations for {dataset_id}: {configs}"
)

for config_name in configs:
if check_memory_usage():
logger.warning(
f"Skipping config {config_name} due to high memory usage"
)
continue

logger.info(
f"Attempting to load config: {config_name}"
)
if dataset_size_kb and dataset_size_kb > 1000000:
success, error, info = load_dataset_in_chunks(
dataset_id, config_name, token
)
else:
success, error, info = try_load_dataset(
dataset_id,
...
result["status"] = "verified"
logger.info(
f"Successfully verified {dataset_id} with config {config_name}"
)
break
else:
result["configs"][
...
logger.error(
f"Failed to verify config {config_name}: {str(error)}"
)
cleanup_memory()

except Exception as e:
logger.error(
f"Failed to get/verify configurations for {dataset_id}: {str(e)}"
)
result["error"] = str(e)
cleanup_memory()
...
log_verification_attempt(
logger, dataset_id, "initial_info", error=e, success=False
)
...
logger.info(
f"Final memory usage: {psutil.Process().memory_percent():.1f}%"
)
cleanup_memory()
return local_dir, result
...
# Sample of important configs
"openai/summarize_from_feedback": ["axis", "comparisons"],

# Will try default config
"textvqa": None,
}
...
for i,(
local_dir,
dataset_id) in enumerate(dataset_items):
)
...
# Size in KB > 1GB
logger.info(
f"Large dataset detected ({dataset_size/1024/1024:.1f} GB). Processing individually: {dataset_id}"
)
batch_size = 1
else:
batch_size = 2
...
1
total_batches = (len(dataset_items) +
batch_size - 1) // batch_size
logger.info(
f"Processing batch {batch_num}/{total_batches}(
Progress: {verified_count}/{total_datasets} verified,                 {failed_count} failed)"
)
)
...
try:
local_dir, result = verify_dataset(
local_dir, dataset_id, token, config
)
...
logger.info(
f"Verified {local_dir} ({dataset_id}) with config {config}: {result['status']}"
)
except Exception as e:
logger.error(
f"Error verifying {dataset_id} with config {config}: {str(e)}"
)
...
= 1
logger.info(
f"Verified {local_dir} ({dataset_id}): {result['status']}"
)
except Exception as e:
...
"failed": sum(
1 for r in verification_results.values() if r["status"] == "failed"
),
"auth_failed": sum(
1
for r in verification_results.values()
