...
def try_load_dataset(dataset_id: str,
...
def log_verification_attempt(logger: logging.Logger,
                           dataset_id: str,
                           attempt_type: str,
                           config: Optional[str] = None,
...
def load_dataset_in_chunks(dataset_id: str,
                          split: str = 'train',
                          chunk_size: int = 50,
        ...
        try:
            ...
            while start_byte < file_size:
                ...
                while retry_count < max_retries and chunk_data is None:
                    try:
                        logging.debug(f"Downloading bytes {start_byte}-{end_byte} ({(end_byte-start_byte+1)/(1024*1024):.2f} MB)")
