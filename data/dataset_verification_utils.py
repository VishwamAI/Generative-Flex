...
class TimeoutException(Exception):
    ...
    **kwargs
    ...
    formatted +
    = f"  Error Category: {attempt['error_category']}\n"
    formatted +
    = "\n"
    ...
    = f"\nFinal Error: {error}\n"
    formatted +
    = f"Error Category: {categorize_error(Exception(error))}\n"
    ...
    logger.error(
    f"Failed to verify {dataset_id}{config_str} using {attempt_type}"
    )
    ...
    end_byte = min(
    start_byte +
    download_chunk_size - 1, file_size - 1
    )
    ...
    try:
    logging.debug(
    f"Downloading bytes {start_byte}-{end_byte} ({(end_byte-start_byte +
    1)/(1024*1024):.2f} MB)"
    )
    ...
    partial_line = (
    lines[-1] if not chunk_data.endswith("\n") else ""
    )
    ...
    * 100,
    ...
    = 1
    logging.warning(
    f"JSON parse error: {str(je)[:100]}..."
    )
    if (
    error_count > chunks_processed * 0.1
    ...
    logging.warning(
    f"Network error, falling back to datasets library: {str(
    re)}"
    )
    )
