...
except Exception as e: logging.error(f"Error parsing JSON: {}")
raise
...
total_objects += 1
if total_objects % 100 == 0: current_memory = get_memory_usage()            logging.info(f"Processed {} objects. Memory usage: {
     current_memory: .1f
 }%")
...
except Exception as e: logging.error(f"Error verifying dataset {}: {}")
return {
     "success": False,
     "error": str(e)
 }
