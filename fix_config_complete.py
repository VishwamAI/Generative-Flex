...
def from_json(self
    cls
    path: st
    r) -> "Config": """Load configuration from JSON file."""                with open(path
...
def save_json(self
                                    path: st
                                    r) -> None: """Save configuration to JSON file."""config_dict = {
"model": {k: vfork
...
def get_config(self
    ...
    [str] = None) -> "Config": """Get configuration for a specific model type."""                if config_path and Path(config_path).exists():
...
try: withopen(config_path
                    "rb") as f: content = f.read()                formatted = black.format_file_contents(content
                    fast=False
                    mode=mode)
with open(config_path
                    "w") as f: f.write(formatted)
