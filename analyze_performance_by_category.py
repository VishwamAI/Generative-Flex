...
total_problems = sum(
cat["total_problems"] for cat in category_stats["categories"].values()
)
...
else (  # Adjust based on problem complexity
0.9 if category == "Geometry" else 1.0
)  # Default weight for Other
)
...
{analysis["overall_metrics"]["accuracy"]*100:.1f}%)',
...
report.append("-" * 30)
report.append(
f"Overall Accuracy: {analysis['overall_metrics']['accuracy']*100:.2f}%"
)
if analysis["overall_metrics"]["validation_loss"]:
report.append(
f"Validation Loss: {analysis['overall_metrics']['validation_loss']:.4f}"
)
...
report.append(
f"  Estimated Accuracy: {data['estimated_accuracy']*100:.2f}%"
)
...
report.append("\nStrengths:")
report.append(
f"- Strongest in {top_category[0]} with {top_category[1]['estimated_accuracy']*100:.2f}% accuracy"
)
report.append(
f"- Represents {top_category[1]['percentage']:.1f}% of validation set"
)

report.append("\nAreas for Improvement:")
report.append(
f"- Needs improvement in {bottom_category[0]} with {bottom_category[1]['estimated_accuracy']*100:.2f}% accuracy"
)
report.append(
f"- Represents {bottom_category[1]['percentage']:.1f}% of validation set"
)
