...
class SimpleChatModel(nn.Module):    vocab_size: int, hidden_size: int = 64
...
def test_response_generation(self, chat_model, model_params, word_mappings) -> None:    """Test end-to-end response generation."""    word_to_id, id_to_word = word_mappings
...
# Generate response
logits = chat_model.apply({"params": model_params}, input_tokens)predicted_tokens = jnp.argsort(logits)[-10:][::-1]
...
def test_unknown_token_handling(self, chat_model, model_params, word_mappings) -> None:    """Test model handling of unknown tokens."""    word_to_id, __ = word_mappings
