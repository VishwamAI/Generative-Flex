...
def remove_all_docstrings(content: str) -> str:
    ...
    content = re.sub(r'(\s*)class\s+\w+.*?:\s*""".*?"""', r'\1class \2:', content, flags=re.MULTILINE | re.DOTALL)
    content = re.sub(r'(\s*)def\s+\w+.*?:\s*""".*?"""', r'\1def \2:', content, flags=re.MULTILINE | re.DOTALL)
