...
name
                    ...
                    1)                params.append(f"{name.strip()}: {type_hint.strip()}")
            else:
                params.append(param)
...
def fix_dataclass_fields(self
        content: st
        r) -> str: """Fix dataclass field definitions."""        lines = []
        for line in content.splitlines():
            if "field(" in line:
                # Split multiple field definitions on the same line
if "
                    " in line and "=" in line: parts = line.split("
                    ")
                    fixed_parts = []
                    for part in parts:
                        if "field(" in part:
                            ...
                            if ":" in name_type:
name
                                    type_hint = name_type.split(": "
                                    1)                                fixed_parts.append(
...
def fix_indentation(self
        content: st
        r) -> str: """Fix indentation while preserving logical structure."""        lines = content.splitlines()
        fixed_lines = []
        indent_level = 0
