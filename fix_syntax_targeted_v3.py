    ...
    def format_params(match):
        ...
        for param in param_list:
            ...
            if '=' in param:
                ...
                value = value.strip()
                if ':' in name:
                    param_name, type_hint = name.split(':', 1)
        ...
        if len(formatted_params) <= 2:
            return f"def {name}({', '.join(formatted_params)}):"
        else:
...
def format_function_call(name: str, args: str) -> str:
    """Format function call with proper line breaks and indentation."""
    args = args.strip()
    if ',' not in args or len(args) < 80:
        return f"{name}({args})"
