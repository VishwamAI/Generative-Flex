...
"""
    ...
    content = re.sub(
        r"from dataset_verification_utils.*?\)", fixed_imports, content, flags=re.DOTALL
    )
    content = re.sub(
        r"basic_strategies = \[.*?\]", fixed_basic_strategies, content, flags=re.DOTALL
    )
    content = re.sub(
        r"dataset_configs = {.*?}", fixed_dataset_configs, content, flags=re.DOTALL
    )
    ...
    try:
        ...
        print(f"Black formatting failed: {str(e)}")
