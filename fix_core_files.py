...
def fix_class_def(content: str) -> str:
    """Fix class definition formatting."""
        ...
        def fix_class_params(match: re.Match) -> str: full_def = match.group(0)
        class_name = match.group(1)
        inheritance = match.group(2)
        
        if not inheritance: returnfull_def# Clean up inheritance list
...
def fix_string_formatting(content: str) -> str:
    """Fix string formatting issues."""
        ...
        # Fix f-strings
        def fix_fstring(match: re.Match) -> str: prefix = match.group(1) or ""
        quote = match.group(2)
