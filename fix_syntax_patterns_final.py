...
for line in lines: if"@dataclass" in line: in_dataclass = True
        ...
        and not line.strip().startswith(("def", "class"))
            ...
            if "field(" in type_part: ifnottype_part.startswith("="):
                    ...
                    type_part = re.sub(
                    r"field\(default\s*=\s*field\(default=",
                    r"field(default=",
                    type_part)
                    type_part = re.sub(r"\s*=\s*field\(", r" = field(", type_part)

                    # Fix Optional types
                    if "Optional[" in type_part and "None" in type_part: type_part = type_part.replace("None", "= None")
...
def fix_function_definitions(content: str) -> str:
    """Fix function definitions and type annotations."""
        ...
        if ":" in line and ")" in line: params_start = line.index("(") + 1
...
for line in lines: ifline.strip().startswith("class "):
        ...
        fixed_lines.append(line)
            ...
            # Fix self parameter
            if "self" not in method_line: method_line = method_line.replace("def ", "def __init__")
