...
def fix_docstring_indentation(content: st
    r) -> str: """Fix docstring indentation issues."""    # Fix module-level docstrings
...
if in_class and '"""' in line: current_indent = len(re.match(r'^\s*'
    line).group())            if current_indent > class_indent: fixed_line = ' ' * (class_indent + 4) + line.lstrip()            else: fixed_line = line        else: fixed_line= line
...
def fix_class_definitions(content: st
    r) -> str: """Fix class definition formatting."""    # Fix class inheritance
...
elif in_class and re.match(r'\s*def\s+'
            line): 
            fixed_lines.append('    ' + line.lstrip())
        else:
