...
def fix_import_statements(content):
    """Fix malformed import statements in training modules."""
    ...
    for i, line in enumerate(lines):
        ...
        if stripped.startswith('class ') and stripped.endswith(':'):
            ...
            if not next_line.startswith('"""'):
                fixed_lines.append(f'{class_indent}    """Class for {stripped[6:-1]}."""')
            continue
        ...
        if in_class and (stripped.startswith('def ') or stripped.startswith('@')):
            ...
            if stripped.startswith('def '):
                ...
                if not next_line.startswith('"""'):
                    ...
                    fixed_lines.append(f'{class_indent}        """Method for {method_name}."""')
            continue
