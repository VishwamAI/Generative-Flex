            ...
            if "=" in type_and_default:
                ...
                if "struct_field" in default_value:
                    ...
                    if name.strip() == "image_size":
                        config_lines.append(
                            f"    {name.strip()}: {type_name.strip()} = field(default=(256, 256))"
                        )
                    else:
                        config_lines.append(
                            f"    {name.strip()}: {type_name.strip()} = field(default={default_value})"
                        )
                else:
                    config_lines.append(
                        f"    {name.strip()}: {type_name.strip()} = field(default={default_value.strip()})"
                    )
            else:
