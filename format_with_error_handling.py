...
try:
    ...
    if result.returncode != 0:
        print(f"Warning: Initial formatting failed for {file_path}")
        print(f"Error: {result.stderr}")

        # Try with more lenient settings
        result = subprocess.run([
        "black", "--target-version", "py39", "--skip-string-normalization", file_path, ], capture_output=True, text=True, check=False)
