...
def extract_problem_category(text):
    ...
    text = text.lower()
    categories = {
        'Algebra': ['algebra', 'equation', 'polynomial', 'variable', 'solve for', 'linear'],
        'Calculus': ['calculus', 'derivative', 'integral', 'differentiate', 'integrate', 'limit'],
        'Probability & Statistics': ['probability', 'statistics', 'random', 'distribution', 'expected value'],
        'Geometry': ['geometry', 'triangle', 'circle', 'angle', 'polygon', 'area'],
        'Number Theory': ['number theory', 'prime', 'factor', 'divisor', 'gcd', 'lcm']
    }
...
def parse_validation_outputs():
    ...
    with open(latest_log, 'r') as f:
        ...
        for block in problem_blocks[1:]:  # Skip the first split as it's before any problem
